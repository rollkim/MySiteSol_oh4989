...
export function makeQueryClient() {
  return new QueryClient({
    defaultOptions: {
      queries: {
        // 매물은 자주 바뀌지 않는다 — 잠깐의 stale을 허용해 지도 이동 중 중복 요청을 줄인다
        staleTime: 30 * 1000,
        /**
         * 4xx는 재시도하지 않는다 — 없는 매물·권한 없음·잘못된 입력은 다시 보내도 같은 답이다.
         * 기본값(3회)이면 오류 화면이 뜨기까지 수 초가 걸려 '로딩이 멈춘 것처럼' 보인다.
         * 모바일 비중이 70%라 이 체감 차이가 크다. 5xx·네트워크 끊김만 두 번까지 다시 시도한다.
         */
