 ...
 *
 * Next의 standalone 폴더에는 src/·.env까지 섞여 들어올 수 있다(추적기가 업로드 경로를
...
const OUT_DIR = path.join(path.dirname(PROJECT_DIR), "oh4989-release");

/**
 * 이름을 고정한다. 날짜를 넣으면 배포할 때마다 손으로 쳐야 하고, 서버에도 릴리스가
 * 무한정 쌓인다. 이전 버전 보관은 서버의 a/b 두 폴더가 맡는다(scripts/server-deploy.sh).
 */
const TARBALL_NAME = "oh4989-release.tar.gz";
const INNER_DIR_NAME = "oh4989";
...
console.log("oh4989 릴리스 묶음\n");

// ── 1. 빌드 ────────────────────────────────────────────────────────────────
console.log("[1/4] Next 빌드");
...
// 이 프로젝트는 이미지를 브라우저 Canvas에서 처리하고 next/image를 쓰지 않으므로
// sharp가 담겨도 호출되지 않는다. 다만 조용히 넘어가면 서버에서 원인 모를 오류로
// 만나게 되므로 반드시 보여 준다. **sharp를 도입하면 이 경고가 실제 장애가 된다.**
...
// 지정으로 해석해 "Cannot connect to C" 로 죽는다.
...
console.log("서버로 보내기:");
console.log(`  scp "${tarball}" oh4989@1.234.79.54:/home/oh4989/`);
console.log("서버에서:");
